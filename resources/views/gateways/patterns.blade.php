@extends('layouts.admin', ['pageTitle' => 'الگوی مصرف درگاه '.$gateway->serial_number, 'newButton' => false])
@section('content')
<style>
    .error {
        ...
        <div class="panel-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody id="table-body">
                    @foreach($gatewayPatterns as $pattern)
                        <tr>
    ...
    <script>
        ...
        function save() {
            ...
            if (count > 0) {
                $.ajax("{{route('gateways.patterns.massStore')}}", {
                    type: "post",
                    data: {
                        _token: "{{csrf_token()}}",
                        gateway: "{{$gateway->id}}",
        ...
        function removeSingleItem(id) {
            swal({
                ...
                if (willDelete) {
                    $.ajax("destroySinglePatternRow/"+id, {
                        type: "get",
                        success: function(response) {
                            if (response.status === 1) {
                                window.location.reload();
